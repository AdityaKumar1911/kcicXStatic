...
import { MapPin, Phone, Mail, Check, Users, GraduationCap, Building2, ArrowRight } from 'lucide-react';
...
export default function CampusDetail() {
  ...
  const handleCTA = () => {
    const btn = document.querySelector('.npfWidgetButton.npfWidget-5cd7c310499045622b34d8ab404bd5be') as HTMLButtonElement;
    if (btn) btn.click();
  };
            ...
            {campusTutors.length > 0 && (
              <Card className="border-0 shadow-xl">
                ...
                <CardContent>
                  <div className="grid sm:grid-cols-2 gap-4">
                    {campusTutors.map(t => (
                      <Link key={t.id} to={`/tutors/${t.slug}`} className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl hover:shadow-lg transition-all group">
          ...
          <div className="space-y-6">
            <Card className="sticky top-24 border-0 shadow-xl bg-gradient-to-br from-[#0B1B3A] to-[#1a2f5a] text-white">
              ...
              <CardContent className="space-y-4">
                <a href="https://admissions.cornerstone.edu.in/" target="_blank" rel="noopener noreferrer"><Button className="w-full bg-[#B99750] hover:bg-[#a88645]">Apply Now</Button></a>
